...
class CulqiValidation:
    ...
    def customer_validation(self, data):
        ...
        if not data.get('address_city'):
            raise Exception('address_city is empty.')
    ...
    def order_validation(self, data):
        ...
        if not client_details.get('phone_number'):
            raise Exception('phone_number is empty.')
        ...
        if not CulqiValidation.is_future_date(data['expiration_date']):
            raise Exception('expiration_date must be a future date.')
    ...
    def validate_currency_code(currency_code):
        ...
        if not isinstance(currency_code, str):
            raise Exception('Currency code must be a string.')
        
        allowed_values = ['PEN', 'USD']
        if currency_code not in allowed_values:
            raise Exception('Currency code must be either "PEN" or "USD".')

    def validate_string_start(string, start):
        if not string.startswith(start + "_test_") and not string.startswith(start + "_live_"):
            raise Exception(f'Incorrect format. The format must start with {start}_test_ or {start}_live_')

    def validate_value(value, allowed_values):
        if value not in allowed_values:
            raise Exception(f'Invalid value. It must be {json.dumps(allowed_values)}.')
